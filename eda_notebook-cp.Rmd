...
# Summary statistics
fk_by_subj %>%
    summarise(avg_title_len = mean(title_length) %>% signif(digits = 1),
...
bs_summ <- bs_clean %>% 
    group_by(site_url) %>% 
    summarise(count = n(),
              avg_title_length = mean(title_length) %>% signif(digits = 2),
              avg_text_length = mean(text_length) %>% signif(digits = 2),
              stdev_title_length = sd(title_length) %>% signif(digits = 2),
              avg_spam = mean(spam_score) %>% signif(digits = 2),
              avg_replies = mean(replies_count) %>% signif(digits = 3),
              avg_participants = mean(participants_count) %>% signif(digits = 3),
              avg_likes = mean(likes) %>% signif(digits = 3),
              avg_comments = mean(comments) %>% signif(digits = 3),
              shares = mean(shares) %>% signif(digits = 3),
