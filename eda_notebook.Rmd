...
```
```{r Kaggle-fake-data-a, echo=FALSE}
# Summary statistics
fk_by_subj %>%
...
bs_detector <- read_csv('data/fake-kaggle.csv', 
                        col_types = 'cdf?ccfccfdcdcdddddf')
...
bs_clean <- bs_detector %>% 
    ...
    dplyr::mutate(domain_name = parse_factor(domain_name), 
           tld = parse_factor(tld),
           main_img_domain = str_extract(main_img_url, domain_extract),
           title_length = text_split(title), text_length = text_split(text),
           published = as.Date(published)) %>%
    dplyr::arrange(published, country, language)
bs_domains <- bs_clean %>%
    select(domain_name, tld, domain_rank, spam_score) %>%
    mutate(domain_name = paste(domain_name, '.', tld, sep = '')) %>%
    group_by(domain_name) %>%
    summarise(avg_rank = mean(domain_rank), 
              avg_spam = round(mean(spam_score), 2))
```



```{r bs_detector-timeseries, echo=FALSE, warning=FALSE}
dates <- bs_clean %>% group_by(published) %>% summarise(count = n())
ggplot(dates, aes(published, count)) +
    geom_line(color = BASE_FILL) +
    scale_x_date(date_labels = '%b %d', date_minor_breaks = '1 day',
                 date_breaks = '3 day') +
    theme(axis.text.x=element_text(angle=60, hjust=1)) +
    ggtitle('Fake article publication by date',
            subtitle = 'Dates from 25 October 2016 to 25 November 2016') +
    labs(x = '', y = 'Articles published',
         caption = paste('Source: ', KAGGLE_BS_LINK)) +
    theme(plot.title = element_text(face = 'bold')) +
    theme(plot.subtitle = element_text(face = 'bold', color = 'grey35')) +
    theme(plot.caption = element_text(color = 'grey68')) +
    geom_vline(xintercept = as.Date('2016-11-08'), color = 'orange', size=.5) +
    annotate(geom="text", x=as.Date('2016-11-05'), y=1000, 
             label='Election Day')
```
